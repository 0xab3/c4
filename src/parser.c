...
enum CX_ParseError cxp_parse_proc_args(struct CX_Lexer *lexer,
  ...
  bool ok = cxp_expect(lexer, CX_TOKEN_PAREN_OPEN, nullptr);
  if (!ok) return CXPE_UNEXPECTED_TOKEN;

  while (cxl_token_peek(lexer).kind != CX_TOKEN_PAREN_CLOSE) {
    struct CX_Token temp_token = cxl_token_peek(lexer);
    struct CX_Token name = {};
    struct CX_Type type = {};
    bool ok = cxp_expect(lexer, CX_TOKEN_IDENTIFIER, &name);
    if (!ok) return CXPE_UNEXPECTED_TOKEN;

    ok = cxp_expect(lexer, CX_TOKEN_COLON, nullptr);
    if (!ok) return CXPE_UNEXPECTED_TOKEN;

    enum CX_ParseError err = cxp_parse_type(lexer, &type);
...
enum CX_ParseError cxp_parse_proc(struct CX_Lexer *lexer, struct CX_Procedure *proc) {
  bool ok = cxp_expect(lexer, CX_TOKEN_PROCDECL, nullptr);
  if (!ok) return CXPE_UNEXPECTED_TOKEN;
  ...
  enum CX_ParseError err = cxp_parse_type(lexer, &return_type);
  if (err != CXPE_OK) return err;
  ...
  while (token_ok) {
    ...
    switch (token.kind) {
      ...
      case CX_TOKEN_PROCDECL   : {
        ...
        enum CX_ParseError err = cxp_parse_proc(lexer, &proc);
        if (err != CXPE_OK) {
